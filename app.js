...
async function handleContactForm(e) {
  ...
  try {
    ...
    btn.disabled = false;
    btn.textContent = originalText;
    const mailtoLink = `mailto:nexora.dev7@gmail.com?subject=${encodeURIComponent('Portfolio Message from ' + name)}&body=${encodeURIComponent('Name: ' + name + '\nEmail: ' + email + '\n\nMessage:\n' + message)}`;
    const sendViaEmail = confirm('Could not send via web. Click OK to send via your email client instead.');
    if (sendViaEmail) {
