...
const JSONBIN_MASTER_KEY = '$2b$10$L8yzB0NZ8Z0Z0Z0Z0Z0Z0Z';
const JSONBIN_BIN_ID = 'fahad_portfolio_data';
...
async function syncToGlobal(avatarUrl, resumeData, cvData) {
  try {
    const binId = localStorage.getItem('portfolio_bin_id') || 'fahad_portfolio_main';
    const data = {
      avatarUrl: avatarUrl || '',
    ...
    const response = await fetch(updateUrl, {
      ...
      headers: {
        ...
        'X-Master-Key': '$2b$10$m8Z7Q9X1P2K3L4M5N6O7P8'
      },
    ...
    if (!response.ok) {
      ...
      if (createResp.ok) {
        ...
        localStorage.setItem('portfolio_bin_id', result.metadata.id);
      }
    }
  } catch(e) {
    console.log('Global sync skipped');
  }
}
...
async function handleContactForm(e) {
  ...
  try {
    const response = await fetch('https://formspree.io/f/xdkzojdd', {
      method: 'POST',
      body: formData
    });
    ...
    alert('Message could not be sent. Please try again or email directly to: nexora.dev7@gmail.com');
  }
}
...
async function handleDocUpload(event, type) {
  ...
  try {
    ...
    const avatarUrl = await dbGet('avatar');
    await syncToGlobal(avatarUrl, 
      type === 'resume' ? docStore.resume : await dbGet('doc_resume'),
      type === 'cv' ? docStore.cv : await dbGet('doc_cv')
    );
  } catch(e) { alert('Upload failed: ' + e.message); }
  event.target.value = '';
}
...
async function removeDoc(type) {
  ...
  const avatarUrl = await dbGet('avatar');
  await syncToGlobal(avatarUrl,
    type === 'resume' ? null : await dbGet('doc_resume'),
    type === 'cv' ? null : await dbGet('doc_cv')
  );
}
...
async function handleAvatarUpload(event) {
  ...
  try {
    ...
    await syncToGlobal(url, docStore.resume, docStore.cv);
  } catch(e) { alert('Upload failed: ' + e.message); }
}
...
function resetSite() {
  ...
  localStorage.removeItem('portfolio_bin_id');
  location.reload();
}
...
window.addEventListener('load', async function() {
  ...
  const globalData = await loadGlobalData();
  if (globalData) {
    if (globalData.resume) docStore.resume = globalData.resume;
    if (globalData.cv) docStore.cv = globalData.cv;
  }
