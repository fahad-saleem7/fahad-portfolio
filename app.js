...
const CLOUDINARY_CLOUD  = 'dje6y9h7p';
...
const JSONBIN_ID  = '6a31c12dda38895dfecc8128';
const JSONBIN_KEY = '$2a$10$BIhnDsFesU.wRVEGBOGW3OXuOlyY65kDywXPAbFxisIhFRirDorzi';
...
async function loadAll() {
  try {
    ...
    const allData = await binRead();

    Object.keys(allData).filter(k => k.startsWith('cert_')).forEach(k => {
      const url = allData[k];
      if (!url) return;
      const uid  = k.substring(5);
    ...
    Object.keys(allData).filter(k => k.startsWith('edu_')).forEach(k => {
      const url = allData[k];
      if (!url) return;
      const uid = k.substring(4);
      const img = document.querySelector('img[data-uid="' + uid + '"]');
      if (img) { img.src = url; img.style.display = 'block'; }
    });

    Object.keys(allData).filter(k => k.startsWith('proj_')).forEach(k => {
      const url = allData[k];
      if (!url) return;
      const uid  = k.substring(5);
      const card = document.querySelector('.project-card[data-proj-id="' + uid + '"]');
      if (card) {
...
function handleContactForm(e) {
  ...
  // Validation
  if (!name)    { alert('Please enter your name.');    document.getElementById('contactName').focus(); return; }
  if (!email)   { alert('Please enter your email.');   document.getElementById('contactEmail').focus(); return; }
  if (!message) { alert('Please enter your message.'); document.getElementById('contactMessage').focus(); return; }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { alert('Please enter a valid email address.'); document.getElementById('contactEmail').focus(); return; }
  
  const subject  = encodeURIComponent('Portfolio Message from ' + name);
  const body     = encodeURIComponent('Name: ' + name + '\nEmail: ' + email + '\n\n' + message);
  const gmailUrl = 'https://mail.google.com/mail/?view=cm&fs=1&to=nexora.dev7@gmail.com&su=' + subject + '&body=' + body;

  const btn = e.target.querySelector('button[type="submit"]');
  if (btn) { btn.disabled = true; btn.textContent = 'Opening Gmail...'; }
  ...
  setTimeout(() => {
    ...
    if (n) n.value = ''; if (em) em.value = ''; if (m) m.value = '';
  }, 1500);
}
