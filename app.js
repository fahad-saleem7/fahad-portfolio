...
function loadGlobalData() {
  try {
    const data = {};
    const avatarUrl = localStorage.getItem('portfolio_avatar_url');
    const resume = localStorage.getItem('portfolio_doc_resume');
    const cv = localStorage.getItem('portfolio_doc_cv');
    
    if (avatarUrl) data.avatarUrl = avatarUrl;
    if (resume) data.doc_resume = JSON.parse(resume);
    if (cv) data.doc_cv = JSON.parse(cv);
...
function saveGlobalData(data) {
  try {
    if (data.avatarUrl) localStorage.setItem('portfolio_avatar_url', data.avatarUrl);
    if (data.doc_resume) localStorage.setItem('portfolio_doc_resume', JSON.stringify(data.doc_resume));
    if (data.doc_cv) localStorage.setItem('portfolio_doc_cv', JSON.stringify(data.doc_cv));
  } catch(e) { }
}
...
async function loadAll() {
  try {
    ...
    let data = await dbGet('content_v5');
    ...
    if (globalData && globalData.avatarUrl) {
      ...
      if (img) img.src = globalData.avatarUrl;
      const avatar = document.getElementById('avatar');
      if (avatar) avatar.classList.add('has-img');
      await dbSet('avatar', globalData.avatarUrl);
    } else {
...
async function handleContactForm(e) {
  ...
  try {
    if (typeof emailjs !== 'undefined') {
      const response = await emailjs.send('service_3hs6t8r', 'template_k9f8n7m', {
        to_email: 'nexora.dev7@gmail.com',
        from_name: name,
        from_email: email,
        message: message
      });
    ...
    alert('Message could not be sent. Please try again or contact directly via email.');
  }
}
...
async function handleDocUpload(event, type) {
  ...
  try {
    ...
    saveGlobalData({ ['doc_' + type]: docStore[type] });
  } catch(e) { alert('Upload failed: ' + e.message); }
  event.target.value = '';
}
...
function updateAvatarBtn() {
  ...
  const avatar = document.getElementById('avatar');
  if (btn && avatar.classList.contains('has-img')) {
...
function resetSite() {
  ...
  localStorage.removeItem('portfolio_avatar_url');
  localStorage.removeItem('portfolio_doc_resume');
  localStorage.removeItem('portfolio_doc_cv');
  location.reload();
}
...
window.addEventListener('load', async function() {
  ...
  script.onload = () => {
    if (typeof emailjs !== 'undefined') {
      emailjs.init('hMVWXb49M4gM1d_L5');
    }
  };
  ...
  const globalData = loadGlobalData();
  if (globalData) {
    if (globalData.doc_resume) docStore.resume = globalData.doc_resume;
    if (globalData.doc_cv) docStore.cv = globalData.doc_cv;
  }
